...
def classify(xSample, ySample):
    ...
    f1avg = 0       #will average scores to get a better picture of accuracy
    betaavg = 0
    count = 0

    while(count < 10):
        f1avg += f1_score(ytest, ypred, average='macro')                
        betaavg += fbeta_score(ytest, ypred, average='macro', beta=2.0) #weights recall over precision
        count += 1
    

    print("\nF1 Score Average Over 10 Iterations:", f1avg/10)
    print("F2 Score Average Over 10 Iterations:", betaavg/10)
...
# count_vect.set_params(tokenizer=tokenizer.tokenize)          #Using tfidf_vectorizer instead of tfidf transformer results in marginal accuracy increase
...
xOver, yOver = ros.fit_resample(X_tfidf, lbls)
